...
def see_resource(grid, news: ViewResource, update_chat_box, force_update_grid):
	new_cell = news.get_cell()
	if new_cell is None:
		return
	x, y = new_cell.x, new_cell.y
	...
	if grid.model_cell[x][y]:
		grid.update_vertex_in_graph(Cell(x, y))  # it is not model cell
	if update_chat_box:
		grid.chat_box_writer.report(ViewResource(grid.model_cell[x][y]))  # or only pass new information we saw?
